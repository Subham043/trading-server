  ...
  shareHolderMaster.project?.shareCertificateMasters.forEach(
    async (shareCertificateMaster) => {
      ...
      payload["distinctiveNos"] = shareCertificateMaster.folios
        .map((folio) => folio.distinctiveNosFrom + "-" + folio.distinctiveNosTo)
        .join(", ");
      ...
      shareHolderMaster.legalHeirDetails.forEach((legalHeirDetail, index) => {
        ...
        payload["isDeceased_" + (index + 1)] = legalHeirDetail.isDeceased;
        payload["isMinor_" + (index + 1)] = legalHeirDetail.isMinor;
        payload["isTestate_" + (index + 1)] = legalHeirDetail.isTestate;
