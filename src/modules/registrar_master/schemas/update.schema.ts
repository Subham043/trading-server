import { z } from "zod";
import { getById, getByCompanyId } from "../registrar_master.repository";
import { getById as getByMainComanyId } from "../../company_master/company_master.repository";
...
export const updateRegistrarMasterUniqueSchema = z
  .object({
    id: z
      .number({
    ...
    companyId: z
      ...
      .superRefine(async (companyId, ctx) => {
        const companyMaster = await getByMainComanyId(companyId);
        if (!companyMaster) {
          ctx.addIssue({
            code: "custom",
  ...
  .superRefine(async ({ id, companyId }, ctx) => {
    const registrarMaster = await getByCompanyId(companyId);
    if (registrarMaster && registrarMaster.id !== id) {
      ctx.addIssue({
        code: "custom",
